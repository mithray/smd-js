...
function generateLanguageTransform(inputLanguage, outputLanguage){
  if(debug) console.time('GENERATED LANGUAGE TRANSFORM')
  ...
  if(languageDefinitionAccess){
    ...
    if(debug) console.timeEnd('GENERATED LANGUAGE TRANSFORM')
    return transform
  }
  return
}
function stitch(blocks, original){
  ...
  if(debug) console.time('STITCHED_BLOCK_' + rand)
  ...
  if(debug) console.timeEnd('STITCHED_BLOCK_' + rand)
  return newHtml
}
