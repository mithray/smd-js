...
function generateCategoriesFile(data){
  ...
  console.log(c.white('Writing category list to file'))
  fs.writeFileSync(path.join(__dirname,'../rules/categories.json'), json)
}

console.log(c.white('Fetching WHATWG HTML standard source file...'))
fetch(whatwgSourceUrl)
  .then(res => {
    return res.text()
  })
  .then(body => {
    console.log(c.white('Generating category list'))
